...
GateResult PredatorGate::birth_token(uint32_t sector_id) noexcept {
    ...
    auto* fuel = get_or_create_fuel(sector_id);
    ...
    uint32_t sector_live = live_count_.count(sector_id) ? live_count_[sector_id] : 0;
    if (sector_live >= MAX_TOKENS_PER_SECTOR) {
    ...
    Token::derive_hmac(gate_key_.data(), sector_id, born_at_ns, nonce.data(), token_value.data());
    
    Token token{token_value, nonce, sector_id, born_at_ns, seq, TokenStatus::LIVE,
                std::chrono::steady_clock::now()};
    
    TokenKey key{token_value};
    tokens_[key] = token;
    live_count_[sector_id]++;
    stats_.record_birth(sector_id);
    
    return {true, token_value, std::nullopt, fuel->strike_count.load(std::memory_order_acquire), false};
}

GateResult PredatorGate::admit(uint32_t sector_id, const uint8_t* token_value) noexcept {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (token.sector_id != sector_id) return reject(RejectReason::WRONG_SECTOR, fuel);
    ...
    auto& seen = seen_sequences_[sector_id];
    if (seen.count(token.sequence)) {
        return reject(RejectReason::REPLAYED, fuel);
    }
    
    if (!Token::verify_hmac(gate_key_.data(), token_value, token.sector_id,
    ...
    fuel->reset_strikes();
    stats_.record_admit();
    decrement_live_count(token.sector_id);
...
void PredatorGate::remove_session(uint32_t sector_id) noexcept {
    std::lock_guard<std::mutex> lock(mutex_);
    fuel_flows_.erase(sector_id);
    seen_sequences_.erase(sector_id);
}
...
void PredatorGate::decrement_live_count(uint32_t sector_id) noexcept {
    auto it = live_count_.find(sector_id);
...
FuelFlow* PredatorGate::get_or_create_fuel(uint32_t sector_id) noexcept {
    auto it = fuel_flows_.find(sector_id);
    if (it == fuel_flows_.end()) {
        it = fuel_flows_.emplace(sector_id, std::make_unique<FuelFlow>()).first;
    }
